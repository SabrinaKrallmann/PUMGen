 ...
 */
	...
	private:
		...
		std::map<int, int> id_map; //matches element-id in .neu file to local id, Key = id in .neu file, Value = local id
	...
	public:
		...
		void readElements(unsigned int start, unsigned int count, int* elements)
		{
			...
			m_mesh.seekg(m_elements.seekPosition + start * m_elements.lineSize);
			...
			for (unsigned int i = 0; i < count; i++) {
				...
				getline(m_mesh, line);
				std::istringstream mesh_String(line);

				int key; //local id
				mesh_String >> key;
				int six; //a value that appears in every file but is not used again
				mesh_String >> six;
				int four;  //a value that appears in every file but is not used again
				mesh_String >> four;


				id_map.insert(std::pair<int, int>(key, value)); //inserts an element, using his original id as key and his local id as value
				value++; //incremented, so the next id is different and again unique


				for (int j = 0; j < 4; j++) {
					mesh_String >> elements[i * 4 + j];
					elements[i * 4 + j]--;
					mesh_String.seekg(1, std::stringstream::cur); // White space
				}
		...
		void readGroups(unsigned int start, unsigned int count, ElementGroup* groups)
		{
			...
			for (unsigned int i = 0; i < count; i++) {
				...
				ss >> groups[i].element;//read element id from file



				int key = groups[i].element; //save parsed id to key

				groups[i].element = id_map[key]; //lookup local id by using the parsed id as key
...
#pragma omp parallel for schedule(static)
		...
		void readBoundaries(unsigned int start, unsigned int count, GambitBoundaryFace* boundaries)
		{
			...
			for (unsigned int i = 0; i < count; i++) {
				...
				int key = boundaries[i].element; //save parsed element id to key

				boundaries[i].element = id_map[key]; //lookup local id by using parsed element id
